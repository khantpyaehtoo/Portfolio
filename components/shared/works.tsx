...
export default function StickyWorkSection() {
    ...
    return (
        <section
            id="works"
            ref={containerRef}
            className="w-full bg-[#0a0a0a] text-white px-6 md:px-16 py-24 border-t border-white/5 relative"
        >
            <div className="max-w-5xl mx-auto space-y-16">
                {/* Header */}
                <div className="flex flex-col md:flex-row md:items-end justify-between gap-4">
                    <div>
                        <span className="works-header-anim block text-amber-300 font-mono text-xs uppercase tracking-widest">
                            {"// Selected Works"}
                        </span>
                        <h2 className="works-header-anim text-4xl md:text-6xl font-bold tracking-tight font-doppio mt-1">
                            Crafted with Code
                        </h2>
                    </div>
                    <p className="works-header-anim text-gray-400 text-sm max-w-sm">
                ...
                <div className="space-y-6">
                    <div className="flex flex-wrap gap-2">
                        {project.tags.map((tag, tIndex) => (
                            <span
                                key={tIndex}
                                className="text-xs font-mono px-3 py-1 bg-white/10 text-gray-200 rounded-full border border-white/10 hover:border-amber-300/50 hover:text-amber-300 transition-colors duration-200"
                            >
                                {tag}
                            </span>
                        ))}
                    ...
                    <div className="flex items-center gap-4 pt-2">
                        <a
                            ...
                            className="inline-flex items-center gap-2 bg-white text-black hover:bg-amber-300 font-medium text-sm px-5 py-2.5 rounded-full transition-all duration-200 cursor-pointer active:scale-95"
                        >
                            Live Demo
                            <i className="fa-solid fa-arrow-up-right-from-square font-medium text-xs" />
                        </a>
                        {project.github && (
                            <a
                                href={project.github}
                                target="_blank"
                                rel="noopener noreferrer"
                                className="p-2.5 rounded-full bg-white/5 border border-white/10 text-white hover:bg-white/20 transition-colors cursor-pointer active:scale-95"
                            >
